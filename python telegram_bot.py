    ...
    if __name__ == "__main__":
        main()
    ```
* Sayfanın en altına inip yeşil **"Commit new file"** butonuna tıklayarak kaydedin.

**Adım 3: `requirements.txt` Dosyasını Oluşturma**
* Deponuzun ana sayfasına geri dönün.
* Tekrar **"Add file" -> "Create new file"** butonuna tıklayın.
* Dosya adı kutucuğuna `requirements.txt` yazın.
* Yazı alanına aşağıdaki iki satırı kopyalayıp yapıştırın:
    ```
    python-telegram-bot
    requests
    ```
...
**BÖLÜM 2: Sunucuyu Ayarlamak (Railway)**

Bu bölümde, GitHub'a yüklediğimiz kodları Railway'e tanıtıp çalıştıracağız.

**Adım 4: Railway Projesi Oluşturma**
* [railway.app](https://railway.app) adresine gidin ve GitHub hesabınızla giriş yapın.
* **"New Project"** -> **"Deploy from GitHub repo"** yolunu izleyin.
* GitHub'ı bağlayın ve az önce oluşturduğunuz `TELGRAF-BOTU` deposunu listeden seçin.

**Adım 5: Gizli Bilgileri Ekleme**
* Projeniz oluşturulduktan sonra, **"Variables"** sekmesine gidin.
* **"+ New Variable"** butonuna tıklayarak aşağıdaki **üç değişkeni** teker teker ekleyin. Her birini eklerken "Değişken Adı" ve "Değer" kısımlarını doğru girdiğinizden emin olun:

| Değişken Adı (Name) | Değer (Value) |
| :--- | :--- |
...
**Adım 6: Başlatma Komutunu Vermek (HATANIN ÇÖZÜMÜ)**
* Projenizin kontrol panelinde **"Settings"** sekmesine gidin.
* Sayfada biraz aşağı inip **"Service"** başlığının altındaki **"Start Command"** kutucuğunu bulun.
* Bu kutucuğun içine tam olarak şunu yazın:
    ```
